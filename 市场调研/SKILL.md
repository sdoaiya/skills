...
7. 优先复制并填写 `templates/` 下的标准模板，至少包含 `sources.csv`、`chart-plan.csv`、`hs-scope.csv`、`country-scorecard.csv` 和 `report-outline.md`。
8. 交付前运行 `checks/` 下的校验脚本，确认来源、图表、关键数字和交付文件齐全。
9. 如果需要快速起项目，运行 `scripts/init_project.py` 自动创建目录、复制模板并生成任务说明。
10. 如果需要整理正式交付包，运行 `scripts/package_delivery.py` 汇总 `output/`、`data/`、`charts/`、`sources/` 并生成 ZIP 归档。
11. 如果需要一条命令跑完全部基础校验，运行 `scripts/run_checks.py`。
