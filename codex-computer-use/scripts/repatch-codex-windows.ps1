...
param(
  ...
  [switch]$SkipWorkspaceDependencies,
  [switch]$RegisterMarketplaceOnly,
  [switch]$ForceRebuild
)

$ErrorActionPreference = 'Stop'
$LogPrefix = '[codex-computer-use]'
...
$WorkspaceDependenciesScript = Join-Path $ScriptRoot 'install-workspace-dependencies-local.ps1'
...
function Invoke-WorkspaceDependenciesInstaller {
  param(
    [string]$Stage,
    [switch]$VerifyOnly
  )

  if (-not (Test-Path -LiteralPath $WorkspaceDependenciesScript)) {
    throw "Workspace dependencies installer not found: $WorkspaceDependenciesScript"
  }

  $args = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $WorkspaceDependenciesScript)
  $mode = 'install'
  if ($VerifyOnly) {
    $args += '-VerifyOnly'
    $mode = 'verify/repair'
  ...
  Write-Log "Workspace dependencies ${mode}: $Stage"
  Invoke-Checked 'powershell' $args "Workspace dependencies $mode failed"
}
...
if (-not $SkipWorkspaceDependencies) {
  if ($DryRun) {
    Invoke-WorkspaceDependenciesInstaller -Stage 'preflight before MSIX dry run' -VerifyOnly
  } else {
    Invoke-WorkspaceDependenciesInstaller -Stage 'preflight before MSIX patch'
  }
}
...
if (-not $SkipWorkspaceDependencies) {
  Invoke-WorkspaceDependenciesInstaller -Stage 'final verification' -VerifyOnly
}
