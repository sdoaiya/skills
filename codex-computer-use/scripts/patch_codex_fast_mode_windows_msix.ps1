...
function Copy-DirectoryContentByStream {
  param(
    [string]$Source,
    [string]$Destination
  )
  ...
  if (Test-Path -LiteralPath $Destination) {
    Remove-DirectoryRobust -Path $Destination -RequiredRoot (Split-Path -Parent $Destination)
  }
  ...
  while ($pending.Count -gt 0) {
    ...
    try {
      ...
      Write-Log "warning: skipping unreadable source directory: $current"
      continue
    }

    foreach ($item in $children) {
      if (($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -ne 0) {
        Write-Log "warning: skipping source reparse point: $($item.FullName)"
        continue
      }
      ...
      if ($item.PSIsContainer) {
        ...
        Copy-FileContentByStream $item.FullName $target
      }
    }
  }
}
...
function Copy-PackageLayout {
  ...
  if (-not (Test-Path -LiteralPath $WorkPackageRoot)) {
    ...
    Copy-DirectoryContentByStream $SourcePackageRoot $WorkPackageRoot
  } else {
...
function Find-PatchTargets {
  ...
  if ([string]::IsNullOrWhiteSpace($codexMobileSetupFlowTarget)) {
    Write-Log 'warning: Codex mobile setup-flow auth fallback target not found; skipping optional mobile fallback patch'
    $codexMobileSetupFlowTarget = '__none__'
  }
